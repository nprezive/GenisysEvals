  ...
  <div class="row clearfix p-1">
          ...
          <q-list no-border>
            <template v-for="cSemester in semester" v-if="cSemester.isActive">
              <q-item v-for="c in cSemester.classes">
                <q-item-main>
                  <q-item-tile label>
                    {{ c.className }} {{cSemester.semester}}
                    <q-checkbox v-model="c.isPublic" label="Public Access" />
                    <q-checkbox v-model="testBool" label="Dean Access" />
                  </q-item-tile>
                </q-item-main>
              </q-item>
            </template>
    ...
    <q-modal ref="basicModal" :content-css="{padding: '50px'}">
      ...
      <q-list no-border>
        <q-item v-for="c in cSemester.classes">
          <q-item-main>
                  <q-item-tile label>{{ c.className }} {{cSemester.semester}} <q-checkbox v-model="checked" val="true" label="Public Access" /> <q-checkbox v-model="checked" val="true" label="Dean Access" /></q-item-tile>
          </q-item-main>
        </q-item>
      </q-list>
      </template>
