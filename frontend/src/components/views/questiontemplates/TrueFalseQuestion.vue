    ...
    <div class="row col-xl-4 col-lg-6 col-md-6 col-sm-6 col-xs-6">
      <div class="col-xl-10 col-lg-10 col-md-10 col-sm-12 col-xs-12 ml-auto mr-auto">
        <q-select
          ...
          @change="typeChange(question.question_type_id, 9)"
        ></q-select>
      </div>
    </div>
    <div class="col-xl-3 col-lg-2 col-md-2 col-sm-12 col-xs-12">
      <q-checkbox v-model="question.settings.answer" checked-icon="fa-check" unchecked-icon="fa-times" :label="AnswerLabel"></q-checkbox>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      AnswerLabel () {
        if (this.question.settings.answer) {
          return 'True'
        } else {
          return 'False'
        }
