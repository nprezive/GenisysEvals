    ...
    <div class="row col-xl-4 col-lg-6 col-md-6 col-sm-6 col-xs-6">
      <div class="col-xl-10 col-lg-10 col-md-10 col-sm-12 col-xs-12 ml-auto mr-auto">
        <q-select
          ...
          @change="typeChange(question.question_type_id, 1)"
        ></q-select>
      </div>
    </div>
    <div class="col-xl-4 col-lg-12 col-md-12 col-sm-12 col-xs-12">
      <q-list no-border>
        ...
        <draggable v-model="question['settings']['distractors']" @end="distractorMoved">
          <q-item class="cursor-pointer" v-for="(distractor, index) in question['settings']['distractors']"
                  :key="index">
            <q-item-side>
              <q-icon @click="distractor.correct = false" class="cursor-pointer" name="fa-check"
                      v-if="distractor.correct" color="primary"></q-icon>
              <q-icon @click="distractor.correct = true" class="cursor-pointer" name="fa-circle-o" v-else
                      color="tertiary"></q-icon>
            </q-item-side>
            <q-item-main>
          ...
          <q-item>
            <q-item-main>
              <q-field class="col-9" :error="newDistractorTouched && newDistractor.length < 1"
                       error-label="Distractor cannot be blank." helper="Add an Distractor.">
