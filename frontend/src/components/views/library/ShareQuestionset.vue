...
<script>
  ...
  export default {
    name: 'ShareQuestionset',
    props: ['selectedQuestionsets', 'sharedQuestionsets'],
    data () {
      return {
        spinner: false,
        terms: '',
              ...
              handler: function () {
                ...
                that.$http({
                  ...
                  if (that.selectedQuestionsets.length < 2) {
                    Toast.create.warning('Question Set Shared')
                  } else {
                    Toast.create.warning('Question Sets Shared')
                  }
                  that.sharedQuestionsets()
                  that.spinner = false
                }).catch(error => {
                  console.log(error)
                })
