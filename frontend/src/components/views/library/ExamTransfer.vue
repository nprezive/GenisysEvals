  ...
  export default {
    ...
    methods: {
      ...
      selected (item) {
        let that = this
        ActionSheet.create({
          title: 'Once you transfer, you will no longer have access to these exams.',
              ...
              handler: function () {
                ...
                that.$http({
                  ...
                  if (that.selectedExams.length < 2) {
                    Toast.create.warning('Exam Transferred')
                  } else {
                    Toast.create.warning('Exams Transferred')
                  }
                  that.transferredExams(that.selectedExams)
                  that.spinner = false
                }).catch(error => {
                  console.log(error)
                })
